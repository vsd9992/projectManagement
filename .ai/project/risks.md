...
4. **Multi-tenant data isolation**: unresolved — shared-schema-with-tenant-id vs. schema-per-tenant vs. database-per-tenant has real security/compliance/cost implications. Must be decided explicitly during the stack/architecture discussion, not defaulted implicitly by whatever framework is chosen.
5. **Traceability retrofit cost**: audit logging and document versioning are meant to be core platform services from the first schema (`architecture.md`). If early implementation treats them as an afterthought, retrofitting full traceability later is expensive and may leave gaps in historical data.
6. **"Intuitive and agile" UX ambition vs. entity breadth**: the domain model already has ~15 core entities before any UI exists. Real risk of ending up as complex as the enterprise ERPs this product is meant to differentiate against. UI/UX design must actively guard against this — flagged here so it isn't lost by the time UI work starts.
